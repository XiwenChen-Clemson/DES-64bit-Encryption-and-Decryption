function MessageOut = SBOX(message) %SBOX substitution
% Input 48-bit message and 8 subtables
...
MessageOut = []; %Initial output   
    for i = 1:8
        %Convert to binary tables 
        ST{i} = mat2cell(blkproc(st{i},[1,1],@(x) de2bi(x,4,'left-msb')),ones(1,4),ones(1,16)*4);
        %Determine block
        Block_in  = message(6*(i-1)+1:6*i) ;
        %Do S-box
        Block_out(i,:) = ST{i}(bi2de(Block_in(:,[1,6]),'left-msb')+1,bi2de(Block_in(:,[2:5]),'left-msb')+1);
        %Cat Output
        MessageOut =[MessageOut,cell2mat(Block_out(i,:))] ;    
    end
end
